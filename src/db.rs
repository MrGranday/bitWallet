...
pub async fn transfer_fund(
    ...
    if let Some(sender_user) = coll.find_one(doc! {"email": sender_email}).await? {
        if let Some(receiver_user) = coll.find_one(doc! {"email": receiver_email}).await? {
            println!("Both users found");
...
pub async fn deposit_fund(
    ...
    if let Some(user) = coll.find_one(doc! {"email":email}).await? {
        ...
        log_transaction(tx_t_coll, from, &user.email, amount, tx_type).await?;
    } else {
        println!("User not found ");
    }
    Ok(())
